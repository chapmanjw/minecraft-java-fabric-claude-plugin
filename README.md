...
Fifteen skills make up the build pipeline. Each runs on the model best suited
...
`city-planner`, `building-architect`, `engineer`, and `monument-builder`
...
village mechanics, urban zoning, vernacular modules, architectural techniques,
redstone contraptions, sculpting and pixel-art technique, interview scripts,
blueprint rendering — loaded on demand so the detail never bloats context
until it is needed.
...
then coordinates the fifteen builder skills: survey → research → plan → shape →
