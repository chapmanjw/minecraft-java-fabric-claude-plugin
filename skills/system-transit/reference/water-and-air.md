...
- **Scale up** — a marina is many slips off shared walkways; an industrial
  port adds warehouses (`design-building`) and cranes (`design-monument`);
...
- A **lighthouse** marking a harbor entrance is a `design-building` handoff.
...
- **Hangars, terminals, and control towers** → `design-building`; you place
...
- **Bubble elevator (up)** — a water column over **soul sand** generates
  ...
  magma) is an `system-redstone` handoff.
...
- **Piston or slime-block lifts** are redstone contraptions — an `system-redstone`
