...
pre-baked — the `survey-research` skill fetches verified dimensions and reference
...
1. **Research first** for any named monument or creature — invoke
   `survey-research`, get cited dimensions and reference imagery, persist them to
...
Representative entries — confirm dimensions with `survey-research`.
...
- **Statue of Liberty** — a ~46 m copper figure (≈49 blocks with the crown
  ...
  is a `design-building` handoff. Palette: a copper-oxidation gradient (see
...
- **Great Sphinx** — ~73 m long × ~20 m high, a recumbent lion with a human
  head. Sandstone palette. The figure is yours; the plateau is `terrain-shape`.
...
- **Mt. Rushmore** — four ~18 m faces. The **cliff is `terrain-landmark`**;
