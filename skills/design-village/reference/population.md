...
Population is expressed in `plan.toon` as `spawn` steps (the `exec-worker` runs them
...
4. Allow time — claiming is not instant. The `exec-reflect` verifies claims
...
they *function*. Hand the `exec-reflect` a check:
