...
pre-baked — the `survey-research` skill fetches verified data per build. This file
is the **method**, the **entry schema**, and a set of worked examples that show
the schema in use.

## The replica method

1. **Research first.** Invoke `survey-research`. Get verified figures and cite at
...
Representative entries — confirm dimensions with `survey-research` before building.
