...
the `survey-research` skill fetches verified dimensions and reference imagery per
garden. This file is the **method**, the **entry schema**, and worked examples.

## The method

1. **Research first** for a named garden — invoke `survey-research`, get cited
...
- **Versailles parterre** — French formal. A long central axis with
  ...
  axis at ~1:4. Handoffs: `design-monument` for the fountain figures.
...
- **An English landscape park** (a Stourhead-type circuit) — a
  ...
  `terrain-shape` for the lake basin and a belvedere hill; `design-building`
  for the temples.
