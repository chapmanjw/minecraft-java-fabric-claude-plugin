...
pre-baked — the `survey-research` skill fetches verified data per city. This file
...
1. **Research first** for any real-world or named-fictional city — invoke
   `survey-research`, cite reputable sources, and persist the dossier to the
...
4. **Zone, then street, then fill** — see `zoning.md`, `streets.md`,
   `vernacular.md`.
5. **List the landmark handoffs** for `design-building`.
...
- **Signature silhouette** — the 3–5 buildings that make the skyline read.
  Each becomes a `design-building` handoff.
...
Representative entries — confirm specifics with `survey-research`.
