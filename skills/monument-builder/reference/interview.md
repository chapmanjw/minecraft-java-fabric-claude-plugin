...
Pin down the piece without over-asking. Ask in small grouped batches (use
`AskUserQuestion` for the multiple-choice ones), skip what is already clear,
...
- A named monument or creature → invoke `researcher` for dimensions and
  reference images.
- Extra-large (a 200 m+ statue) → offer a reduced scale, or splitting the
  pedestal/plinth off to `building-architect` / `terraforming` so the figure
  itself stays 1:1.
- Carved into a cliff (a Rushmore-style request) → confirm there is a cliff,
  or hand `natural-landmarks` / `terraforming` the job of making one first.
- A figure on a pedestal → confirm whether the pedestal is habitable (a
  `building-architect` handoff) or a plain plinth.
